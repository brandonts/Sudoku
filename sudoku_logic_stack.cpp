...
int Solver(SudokuPuzzle& x)
{
	...
	while(puzzleStack.back().Check_()!=0) //no exit until puzzle complete or unsolvable
	{
		...
		if(puzzleStack.back().Check_()==-1)
		{
			int value,i,j;
			do{
				if(puzzleStack.back()==puzzleStack.front()) return -1;
				value=guessStack.back()[0];
				i=guessStack.back()[1];
				j=guessStack.back()[2];
				guessStack.pop_back();
				puzzleStack.pop_back();
			}while(value==9);

			puzzleStack.push_back(puzzleStack.back());
			puzzleStack.back().Write_(i,j,value+1);
			guessStack.push_back({value+1,i,j});
			change=true;
		}

		while(Elimination(puzzleStack.back())==0)
		{
			change=true;
			#if DEBUG
			elimination_counter++;
		...
		while(BlockLineElimination(puzzleStack.back())==0)
		{
			change=true;
			#if DEBUG
		...
		while(BlockLineValueElimination(puzzleStack.back())==0)
		{
			change=true;
			#if DEBUG
		...
		while(OnlyPossible(puzzleStack.back())==0)
		{
			change=true;
			#if DEBUG
			only_counter++;
		...
		if(change==false)
		{
			...
			for(int i=0; i<9; i++)
			{
				for(int j=0; j<9; j++)
				{
					if(puzzleStack.back().Read_(i,j)==0)
					{
						foundEmptySquare=true;
						int value=1;
						puzzleStack.push_back(puzzleStack.back());
						puzzleStack.back().Write_(i,j,value);
						guessStack.push_back({value,i,j});	

						#if DEBUG
						puzzleStack.back().Display_();
						guess_counter++;
				...
				if(foundEmptySquare==true) break;
			}
		}
